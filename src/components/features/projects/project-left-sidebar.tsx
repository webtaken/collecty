...
export function ProjectLeftSidebar({ project }: ProjectLeftSidebarProps) {
    ...
    const isWidgetActive = (widgetId: string) => pathname?.includes(`/widgets/${widgetId}`);

    return (
        <div className="flex flex-col h-full bg-white border-r border-slate-200 w-64 flex-shrink-0">
            {/* Project Title Area */}
            <div className="p-4 border-b border-slate-100">
                <h2 className="font-semibold text-slate-800 truncate" title={project.name}>
                    {project.name}
                </h2>
                ...
                <div className="px-3 mb-6">
                    ...
                    <nav className="space-y-1">
                        <Link href={`/projects/${project.id}/subscribers`}>
                            <Button
                                ...
                                className={cn(
                                    ...
                                    isActive("/subscribers") && "bg-orange-50 text-orange-700 font-medium"
                                )}
                            >
                                <Users className={cn("w-4 h-4", isActive("/subscribers") ? "text-orange-600" : "text-slate-500 group-hover:text-orange-600")} />
                                Subscribers
                            </Button>
                        </Link>
                        <Link href={`/projects/${project.id}/settings`}>
                            ...
                            >
                                <Settings className={cn("w-4 h-4", isActive("/settings") ? "text-orange-600" : "text-slate-500 group-hover:text-orange-600")} />
                                Settings
                            </Button>
                        </Link>
                    </nav>
                    ...
                    <nav className="space-y-1">
                        {widgets.map((widget) => (
                            <div key={widget.id} className="group relative flex items-center">
                                <Link href={`/projects/${project.id}/widgets/${widget.id}`} className="flex-1 min-w-0">
                                    <Button
                                        variant="ghost"
                                        className={cn(
                                            "w-full justify-start gap-2 pr-8 hover:bg-orange-50 hover:text-orange-600 transition-colors",
                                            isWidgetActive(widget.id) && "bg-orange-50 text-orange-700 font-medium"
                                        )}
                                    >
                                        <Layers className={cn("w-4 h-4 flex-shrink-0 group-hover:text-orange-600", isWidgetActive(widget.id) ? "text-orange-600" : "text-slate-400")} />
                                ...
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem onClick={() => setWidgetToDelete(widget.id)} className="text-red-600 focus:text-red-600">
            ...
            <AlertDialog open={!!widgetToDelete} onOpenChange={(open) => !open && setWidgetToDelete(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action cannot be undone. This will permanently delete the widget and remove it from our servers.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={handleDeleteWidget} className="bg-red-600 hover:bg-red-700">
                            Delete
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
